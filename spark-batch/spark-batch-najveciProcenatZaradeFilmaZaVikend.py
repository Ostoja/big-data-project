#prikazuje najveci procenat zarade filma u odnosu na ukupnu zaradu svih filmova za taj vikend
...
print df.groupBy("year", "calendarWeek").agg(f.sum("boxOffice").alias("sumBoxOffice"), f.max("boxOffice").alias("maxBoxOffice"), (f.max("boxOffice") / f.sum("boxOffice")).alias("percentage"), (f.sum("boxOffice") - f.max("boxOffice")).alias("difference")).orderBy("percentage", ascending = False).show(50)
